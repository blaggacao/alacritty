...
use crate::index::{Column, Direction, Line, Point, Side, Whatever};
...
impl ViModeCursor {
    ...
    pub fn motion<T: EventListener>(mut self, term: &mut Term<T>, motion: ViMotion) -> Self {
        ...
        match motion {
            ...
            ViMotion::Left => {
                buffer_point = expand_wide(term, buffer_point, Direction::Left);
            ...
            ViMotion::Right => {
                buffer_point = expand_wide(term, buffer_point, Direction::Right);
...
fn last<T>(term: &Term<T>, mut point: Point<usize>) -> Point<usize> {
    ...
    point = expand_wide(term, point, Direction::Left);
...
fn first_occupied<T>(term: &Term<T>, mut point: Point<usize>) -> Point<usize> {
    ...
    point = expand_wide(term, point, Direction::Right);
...
fn semantic<T: EventListener>(
    ...
    point = expand_wide(term, point, direction);
...
fn expand_wide<T, P>(term: &Term<T>, point: P, direction: Direction) -> Point<usize>
where
    P: Into<Point<usize>>,
{
    ...
    if cell.flags.contains(Flags::WIDE_CHAR) && direction == Direction::Right {
        point.col += 1;
...
fn advance<T>(term: &Term<T>, point: Point<usize>, direction: Direction) -> Point<usize> {
    if direction == Direction::Left {
        point.sub_absolute(term, Whatever::Clamp, 1)
    } else {
        point.add_absolute(term, Whatever::Clamp, 1)
    }
}
