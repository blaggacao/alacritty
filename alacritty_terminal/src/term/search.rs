use std::cmp::min;
use std::mem;
use std::ops::RangeInclusive;

use regex_automata::{dense, DenseDFA, Error as RegexError, DFA};

use crate::event::EventListener;
use crate::grid::{BidirectionalIterator, Dimensions, GridIterator};
use crate::index::{Column, Direction, Point, Whatever};
use crate::term::cell::{Cell, Flags};
use crate::term::Term;

/// Used to match equal brackets, when performing a bracket-pair selection.
const BRACKET_PAIRS: [(char, char); 4] = [('(', ')'), ('[', ']'), ('{', '}'), ('<', '>')];

pub type Match = RangeInclusive<Point<usize>>;

/// Viewport state before search.
struct PreSearchState {
    /// Grid display offset.
    display_offset: usize,

    /// Global vi cursor position.
    vi_absolute: Point<usize>,
}

/// Terminal regex search state.
pub struct RegexSearch {
    /// Locate end of match searching right.
    pub right_fdfa: DenseDFA<Vec<usize>, usize>,
    /// Locate start of match searching right.
    pub right_rdfa: DenseDFA<Vec<usize>, usize>,

    /// Locate start of match searching left.
    pub left_fdfa: DenseDFA<Vec<usize>, usize>,
    /// Locate end of match searching left.
    pub left_rdfa: DenseDFA<Vec<usize>, usize>,

    /// Search direction.
    pub direction: Direction,

    /// State for restoring the viewport in case search is cancelled.
    pre_search_state: Option<PreSearchState>,
}

impl RegexSearch {
    /// Build the forwards and backwards search DFAs.
    pub fn new(search: &str, direction: Direction) -> Result<RegexSearch, RegexError> {
        // Check case info for smart case
        let has_uppercase = search.chars().any(|c| c.is_uppercase());

        let mut builder = dense::Builder::new();
        let builder = builder.case_insensitive(!has_uppercase);

        let left_fdfa = builder.clone().reverse(true).build(search)?;
        let left_rdfa = builder.clone().anchored(true).longest_match(true).build(search)?;

        let right_fdfa = builder.clone().build(search)?;
        let right_rdfa = builder.anchored(true).longest_match(true).reverse(true).build(search)?;

        Ok(RegexSearch {
            pre_search_state: None,
            right_fdfa,
            right_rdfa,
            left_fdfa,
            left_rdfa,
            direction,
        })
    }
}

impl<T> Term<T> {
    /// Cancel active terminal buffer search.
    #[inline]
    pub fn cancel_search(&mut self) {
        let regex_search = match &mut self.regex_search {
            Some(regex_search) => regex_search,
            None => return,
        };

        if let Some(state) = &mut regex_search.pre_search_state {
            self.grid.display_offset = state.display_offset;
            self.vi_mode_cursor.point = self.grid.clamp_buffer_to_visible(state.vi_absolute);
        }

        self.regex_search = None;
        self.dirty = true;
    }

    /// Active search direction.
    #[inline]
    pub fn search_direction(&self) -> Option<Direction> {
        self.regex_search.as_ref().map(|rs| rs.direction)
    }

    /// Enter terminal buffer search mode.
    #[inline]
    pub fn start_search(&mut self, search: &str, direction: Direction)
    where
        T: EventListener,
    {
        // Make sure previous search state is cleaned up.
        self.cancel_search();

        // Setup DFAs for search.
        self.regex_search = RegexSearch::new(search, direction).ok();

        let pre_search_state = PreSearchState {
            display_offset: self.grid.display_offset,
            vi_absolute: self.visible_to_buffer(self.vi_mode_cursor.point),
        };

        // Jump to the first match, so the viewport is moved.
        self.search_next(direction);

        // Store viewport state before search for recovery.
        if let Some(regex_search) = &mut self.regex_search {
            regex_search.pre_search_state = Some(pre_search_state);
        }
    }

    /// Jump to next search match.
    pub fn search_next(&mut self, direction: Direction)
    where
        T: EventListener,
    {
        // Discard original display offset after manually jumping to any match.
        match &mut self.regex_search {
            Some(regex_search) => regex_search.pre_search_state = None,
            None => return,
        }

        let regex_match = match self.next_match(direction) {
            Some(regex_match) => regex_match,
            None => return,
        };

        // Move viewport to make match visible.
        self.scroll_to_point(*regex_match.start());

        // Move vi cursor to the beginning of the match.
        self.vi_mode_cursor.point = self.grid.clamp_buffer_to_visible(*regex_match.start());

        self.dirty = true;
    }

    /// Get the next regex match in the specified direction.
    fn next_match(&mut self, direction: Direction) -> Option<Match> {
        let vi_absolute = self.visible_to_buffer(self.vi_mode_cursor.point);

        Some(match direction {
            Direction::Left => {
                // Start search at the current vi cursor position.
                let origin = vi_absolute.sub_absolute(self, Whatever::Wrap, 1);
                let start = self.line_search_right(origin);
                let end = start.add_absolute(self, Whatever::Wrap, 1);

                let mut regex_iter = RegexIter::new(start, end, Direction::Left, &self).peekable();

                // Check if there's any match at all.
                let first_match = regex_iter.peek()?.clone();

                regex_iter
                    .find(|regex_match| {
                        let match_start = regex_match.start();

                        // If the start of the match is beyond the origin point, we've found our
                        // match.
                        match_start.line < start.line
                            || match_start.line > origin.line
                            || (match_start.line == origin.line && match_start.col <= origin.col)
                    })
                    .unwrap_or(first_match)
            },
            Direction::Right => {
                // Start search at the current vi cursor position.
                let origin = vi_absolute.add_absolute(self, Whatever::Wrap, 1);
                let start = self.line_search_left(origin);
                let end = start.sub_absolute(self, Whatever::Wrap, 1);

                let mut regex_iter = RegexIter::new(start, end, Direction::Right, &self).peekable();

                // Check if there's any match at all.
                let first_match = regex_iter.peek()?.clone();

                regex_iter
                    .find(|regex_match| {
                        let match_start = regex_match.start();

                        // If the start of the match is beyond the origin point, we've found our
                        // match.
                        match_start.line > start.line
                            || match_start.line < origin.line
                            || (match_start.line == origin.line && match_start.col >= origin.col)
                    })
                    .unwrap_or(first_match)
            },
        })
    }

    /// Find the next regex match to the left of the origin point.
    ///
    /// The origin is always included in the regex.
    pub fn regex_search_left(&self, start: Point<usize>, end: Point<usize>) -> Option<Match> {
        let RegexSearch { left_fdfa: fdfa, left_rdfa: rdfa, .. } = self.regex_search.as_ref()?;

        // Find start and end of match.
        let match_start = self.regex_search(start, end, |cell| cell.prev(), &fdfa)?;
        let match_end = self.regex_search(match_start, start, |cell| cell.next(), &rdfa)?;

        Some(match_start..=match_end)
    }

    /// Find the next regex match to the right of the origin point.
    ///
    /// The origin is always included in the regex.
    pub fn regex_search_right(&self, start: Point<usize>, end: Point<usize>) -> Option<Match> {
        let RegexSearch { right_fdfa: fdfa, right_rdfa: rdfa, .. } = self.regex_search.as_ref()?;

        // Find start and end of match.
        let match_end = self.regex_search(start, end, |cell| cell.next(), &fdfa)?;
        let match_start = self.regex_search(match_end, start, |cell| cell.prev(), &rdfa)?;

        Some(match_start..=match_end)
    }

    // TODO: Unicode is broken
    // TODO: Test wide chars
    //
    /// Find the next regex match.
    ///
    /// This will always return the side of the first match which is farthest from the start point.
    fn regex_search<F>(
        &self,
        start: Point<usize>,
        end: Point<usize>,
        mut iter_fn: F,
        dfa: &impl DFA,
    ) -> Option<Point<usize>>
    where
        F: for<'a, 'r> FnMut(&'a mut GridIterator<'r, Cell>) -> Option<&'a Cell>,
    {
        let last_line = self.total_lines() - 1;
        let last_col = self.num_cols() - 1;

        let mut iter = self.grid.iter_from(start);
        let mut state = dfa.start_state();
        let mut point = iter.point();
        let mut cell = *iter.cell();
        let mut regex_match = None;

        loop {
            // Advance regex parser by one `char`.
            let mut buf = [0; 4];
            cell.c.encode_utf8(&mut buf);
            for c in &buf[..cell.c.len_utf8()] {
                // Since we get the state from the DFA, it doesn't need to be checked.
                state = unsafe { dfa.next_state_unchecked(state, *c) };
            }

            // Handle regex state changes.
            if dfa.is_match_or_dead_state(state) {
                if dfa.is_dead_state(state) {
                    match regex_match {
                        Some(_) => break,
                        None => state = dfa.start_state(),
                    }
                } else {
                    regex_match = Some(point);
                }
            }

            // Stop once we've reached the target point.
            if point == end {
                break;
            }

            // Advance grid cell iterator.
            let new_cell = match iter_fn(&mut iter) {
                Some(&cell) => cell,
                None => {
                    // Wrap around to other end of the scrollback buffer.
                    let start = Point::new(last_line - point.line, last_col - point.col);
                    iter = self.grid.iter_from(start);
                    *iter.cell()
                },
            };
            let last_point = mem::replace(&mut point, iter.point());
            let last_cell = mem::replace(&mut cell, new_cell);

            // Handle linebreaks.
            if (last_point.col == last_col
                && point.col == Column(0)
                && !last_cell.flags.contains(Flags::WRAPLINE))
                || (last_point.col == Column(0)
                    && point.col == last_col
                    && !cell.flags.contains(Flags::WRAPLINE))
            {
                // TODO: This duplicates the dead_state block
                match regex_match {
                    Some(_) => break,
                    None => state = dfa.start_state(),
                }
            }
        }

        regex_match
    }

    /// Find next matching bracket.
    pub fn bracket_search(&self, point: Point<usize>) -> Option<Point<usize>> {
        let start_char = self.grid[point.line][point.col].c;

        // Find the matching bracket we're looking for
        let (forwards, end_char) = BRACKET_PAIRS.iter().find_map(|(open, close)| {
            if open == &start_char {
                Some((true, *close))
            } else if close == &start_char {
                Some((false, *open))
            } else {
                None
            }
        })?;

        let mut iter = self.grid.iter_from(point);

        // For every character match that equals the starting bracket, we
        // ignore one bracket of the opposite type.
        let mut skip_pairs = 0;

        loop {
            // Check the next cell
            let cell = if forwards { iter.next() } else { iter.prev() };

            // Break if there are no more cells
            let c = match cell {
                Some(cell) => cell.c,
                None => break,
            };

            // Check if the bracket matches
            if c == end_char && skip_pairs == 0 {
                return Some(iter.point());
            } else if c == start_char {
                skip_pairs += 1;
            } else if c == end_char {
                skip_pairs -= 1;
            }
        }

        None
    }

    /// Find left end of semantic block.
    pub fn semantic_search_left(&self, mut point: Point<usize>) -> Point<usize> {
        // Limit the starting point to the last line in the history
        point.line = min(point.line, self.total_lines() - 1);

        let mut iter = self.grid.iter_from(point);
        let last_col = self.num_cols() - Column(1);

        while let Some(cell) = iter.prev() {
            if !cell.flags.intersects(Flags::WIDE_CHAR | Flags::WIDE_CHAR_SPACER)
                && self.semantic_escape_chars.contains(cell.c)
            {
                break;
            }

            if iter.point().col == last_col && !cell.flags.contains(Flags::WRAPLINE) {
                break; // cut off if on new line or hit escape char
            }

            point = iter.point();
        }

        point
    }

    /// Find right end of semantic block.
    pub fn semantic_search_right(&self, mut point: Point<usize>) -> Point<usize> {
        // Limit the starting point to the last line in the history
        point.line = min(point.line, self.total_lines() - 1);

        let mut iter = self.grid.iter_from(point);
        let last_col = self.num_cols() - 1;

        while let Some(cell) = iter.next() {
            if !cell.flags.intersects(Flags::WIDE_CHAR | Flags::WIDE_CHAR_SPACER)
                && self.semantic_escape_chars.contains(cell.c)
            {
                break;
            }

            point = iter.point();

            if point.col == last_col && !cell.flags.contains(Flags::WRAPLINE) {
                break; // cut off if on new line or hit escape char
            }
        }

        point
    }

    /// Find the beginning of the current line across linewraps.
    pub fn line_search_left(&self, mut point: Point<usize>) -> Point<usize> {
        while point.line + 1 < self.total_lines()
            && self.grid[point.line + 1][self.num_cols() - 1].flags.contains(Flags::WRAPLINE)
        {
            point.line += 1;
        }

        point.col = Column(0);

        point
    }

    /// Find the end of the current line across linewraps.
    pub fn line_search_right(&self, mut point: Point<usize>) -> Point<usize> {
        while self.grid[point.line][self.num_cols() - 1].flags.contains(Flags::WRAPLINE) {
            point.line -= 1;
        }

        point.col = self.num_cols() - 1;

        point
    }
}

pub struct RegexIter<'a, T> {
    point: Point<usize>,
    end: Point<usize>,
    direction: Direction,
    term: &'a Term<T>,
}

impl<'a, T> RegexIter<'a, T> {
    pub fn new(
        start: Point<usize>,
        end: Point<usize>,
        direction: Direction,
        term: &'a Term<T>,
    ) -> Self {
        Self { point: start, end, direction, term }
    }

    /// Skip one cell, advancing the origin point to the next one.
    fn skip(&mut self) {
        self.point = match self.direction {
            Direction::Right => self.point.add_absolute(self.term, Whatever::Wrap, 1),
            Direction::Left => self.point.sub_absolute(self.term, Whatever::Wrap, 1),
        };
    }

    /// Get the next match in the specified direction.
    fn next_match(&self) -> Option<Match> {
        match self.direction {
            Direction::Right => self.term.regex_search_right(self.point, self.end),
            Direction::Left => self.term.regex_search_left(self.point, self.end),
        }
    }
}

impl<'a, T> Iterator for RegexIter<'a, T> {
    type Item = Match;

    fn next(&mut self) -> Option<Self::Item> {
        if self.point == self.end {
            return None;
        }

        let regex_match = self.next_match()?;

        self.point = *regex_match.end();
        self.skip();

        Some(regex_match)
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    use crate::index::{Column, Direction};
    use crate::term::test::mock_term;

    #[test]
    fn regex_right() {
        #[rustfmt::skip]
        let mut term = mock_term("\
            testing66\r\n\
            Alacritty\n\
            123\r\n\
            Alacritty\r\n\
            123\
        ");

        // Check regex across wrapped and unwrapped lines
        term.regex_search = Some(RegexSearch::new("Ala.*123", Direction::Right).unwrap());
        let origin = Point::new(3, Column(0));
        let last = Point::new(term.total_lines(), term.num_cols());
        let start = Point::new(3, Column(0));
        let end = Point::new(2, Column(2));
        assert_eq!(term.regex_search_right(origin, last), Some(start..=end));
    }

    #[test]
    fn regex_left() {
        #[rustfmt::skip]
        let mut term = mock_term("\
            testing66\r\n\
            Alacritty\n\
            123\r\n\
            Alacritty\r\n\
            123\
        ");

        // Check regex across wrapped and unwrapped lines
        term.regex_search = Some(RegexSearch::new("Ala.*123", Direction::Right).unwrap());
        let origin = Point::new(0, Column(2));
        let last = Point::new(term.total_lines(), term.num_cols() - Column(1));
        let start = Point::new(3, Column(0));
        let end = Point::new(2, Column(2));
        assert_eq!(term.regex_search_left(origin, last), Some(start..=end));
    }

    #[test]
    fn nested_regex() {
        #[rustfmt::skip]
        let mut term = mock_term("\
            Ala -> Alacritty -> critty\r\n\
            critty\
        ");

        // Greedy stopped at linebreak
        term.regex_search = Some(RegexSearch::new("Ala.*critty", Direction::Right).unwrap());
        let start = Point::new(1, Column(0));
        let end = Point::new(1, Column(25));
        let last = Point::new(term.total_lines(), term.num_cols());
        assert_eq!(term.regex_search_right(start, last), Some(start..=end));

        // Greedy stopped at dead state
        term.regex_search = Some(RegexSearch::new("Ala[^y]*critty", Direction::Right).unwrap());
        let start = Point::new(1, Column(0));
        let end = Point::new(1, Column(15));
        let last = Point::new(term.total_lines(), term.num_cols());
        assert_eq!(term.regex_search_right(start, last), Some(start..=end));
    }

    #[test]
    fn start_after_origin() {
        #[rustfmt::skip]
        let mut term = mock_term("testing testing testing\r\ntesting");

        // Start in middle of match should skip to the next one
        term.regex_search = Some(RegexSearch::new("te.*ng", Direction::Right).unwrap());
        let origin = Point::new(1, Column(1));
        //let last = Point::new(term.total_lines(), term.num_cols());
        let last = Point::new(origin.line, origin.col - Column(1));
        let start = Point::new(0, Column(0));
        let end = Point::new(0, Column(6));
        assert_eq!(term.regex_search_right(origin, last), Some(start..=end));
    }

    #[test]
    fn start_before_origin() {
        #[rustfmt::skip]
        let mut term = mock_term("\
            testing\r\n\
            testing testing testing\
        ");

        // Start in middle of match should skip to the next one
        term.regex_search = Some(RegexSearch::new("te.*ng", Direction::Right).unwrap());
        let origin = Point::new(0, Column(21));
        let last = Point::new(term.total_lines(), term.num_cols() - Column(1));
        let start = Point::new(1, Column(0));
        let end = Point::new(1, Column(6));
        assert_eq!(term.regex_search_left(origin, last), Some(start..=end));
    }

    #[test]
    fn no_match_right() {
        #[rustfmt::skip]
        let mut term = mock_term("\
            first line\n\
            broken second\r\n\
            third\
        ");

        term.regex_search = Some(RegexSearch::new("nothing", Direction::Right).unwrap());
        let origin = Point::new(0, Column(4));
        let last = Point::new(term.total_lines() - 1, term.num_cols() - Column(1));
        assert_eq!(term.regex_search_right(origin, last), None);
    }

    #[test]
    fn no_match_left() {
        #[rustfmt::skip]
        let mut term = mock_term("\
            first line\n\
            broken second\r\n\
            third\
        ");

        term.regex_search = Some(RegexSearch::new("nothing", Direction::Right).unwrap());
        let origin = Point::new(2, Column(0));
        let last = Point::new(term.total_lines() - 1, term.num_cols() - Column(1));
        assert_eq!(term.regex_search_left(origin, last), None);
    }

    #[test]
    fn include_linebreak_left() {
        #[rustfmt::skip]
        let mut term = mock_term("\
            testing123\r\n\
            xxx\
        ");

        // Make sure the cell containing the linebreak is not skipped
        term.regex_search = Some(RegexSearch::new("te.*123", Direction::Right).unwrap());
        let origin = Point::new(0, Column(0));
        let start = Point::new(1, Column(0));
        let end = Point::new(1, Column(9));
        let last = Point::new(term.total_lines(), term.num_cols() - Column(1));
        assert_eq!(term.regex_search_left(origin, last), Some(start..=end));
    }

    #[test]
    fn include_linebreak_right() {
        #[rustfmt::skip]
        let mut term = mock_term("\
            xxx\r\n\
            testing123\
        ");

        // Make sure the cell containing the linebreak is not skipped
        term.regex_search = Some(RegexSearch::new("te.*123", Direction::Right).unwrap());
        let origin = Point::new(1, Column(2));
        let last = Point::new(term.total_lines(), term.num_cols());
        let start = Point::new(0, Column(0));
        let end = Point::new(0, Column(9));
        assert_eq!(term.regex_search_right(origin, last), Some(start..=end));
    }

    #[test]
    fn skip_dead_cell() {
        let mut term = mock_term("alacritty");

        // Make sure dead state cell is skipped when reversing
        term.regex_search = Some(RegexSearch::new("alacrit", Direction::Right).unwrap());
        let start = Point::new(0, Column(0));
        let end = Point::new(0, Column(6));
        let last = Point::new(term.total_lines(), term.num_cols());
        assert_eq!(term.regex_search_right(start, last), Some(start..=end));
    }

    #[test]
    fn reverse_search_dead_recovery() {
        let mut term = mock_term("zooo lense");

        // Make sure the reverse DFA operates the same as a forwards DFA
        term.regex_search = Some(RegexSearch::new("zoo", Direction::Right).unwrap());
        let origin = Point::new(0, Column(9));
        let last = Point::new(term.total_lines(), term.num_cols() - Column(1));
        let start = Point::new(0, Column(0));
        let end = Point::new(0, Column(2));
        assert_eq!(term.regex_search_left(origin, last), Some(start..=end));
    }
}

#[cfg(all(test, feature = "bench"))]
mod benches {
    extern crate test;

    use super::*;

    use crate::term::test::mock_term;

    #[bench]
    fn regex_search(b: &mut test::Bencher) {
        let input = format!("{:^10000}", "Alacritty");
        let mut term = mock_term(&input);
        term.regex_search = Some(RegexSearch::new("   Alacritty   ", Direction::Right).unwrap());

        b.iter(|| {
            test::black_box(term.regex_search_right(Point::new(0, Column(0))));
            test::black_box(term.regex_search_left(Point::new(0, Column(0))));
        });
    }
}
